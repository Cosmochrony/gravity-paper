...
\begin{document}
  ...
  $|\partial_t \chi| \leq c_{\mathrm{BI}}$ motivates a
  ...
  \subsection{Setup and admissibility conditions}
    ...
    \begin{remark}
      ...
      make the classification below an unconditional
      ...
      (Lemma~\ref{lem:spectralcarrier}) and the constraints
      on the sector function
  ...
  \subsection{Classification of the tensorial completions}
    ...
    \begin{proof}
      ...
      hence it obeys~(C2); and when one eigenvalue of
      $I + X$ approaches zero the corresponding term
      $h(\lambda_i)$ diverges to $-\infty$ while the
      remaining terms stay bounded, so $\Phi(X) \to 0$ and
      (C3) holds.
    ...
    \begin{remark}[Input required for a uniqueness theorem]
      ...
      can legitimately serve as input.
      ...
      its nonlinear
      deformations~\cite{Boillat1970,Plebanski1970}---could
  ...
  \subsection{Structural interpretation}
    ...
    \begin{remark}
      ...
      conditions---the member whose sector function carries
      no deformation beyond the
      constraints~\eqref{eq:h-unique}; its selection within
      that family is an open problem
      (Remark~\ref{rem:uniqueness-input}).
  ...
  \subsection{Limitations}
    ...
    family~\eqref{eq:general-completer}.

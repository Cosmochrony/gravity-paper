...
\begin{document}
  ...
  \subsection{Setup and admissibility conditions}
    ...
    covariance, infrared, and boundedness properties
    established in the spectral framework of Sections~2--5
    and in~\cite{Beau2026c}.
  ...
  \subsection{Uniqueness of the tensorial completion}
    ...
    \begin{proof}
      ...
      Condition~(C2) requires, at first order:
      $\Phi(I + \varepsilon X)
      ...
      In the scalar Born--Infeld theory of~\cite{Beau2026c},
      the one-dimensional completer is uniquely fixed by the
      saturation bound: the action density takes the form
      $\sqrt{1 + \lambda} - 1$ for a single mode of normalised
      field gradient~$\lambda$.
      The associated coherence density is
      $\log \sqrt{1 + \lambda} = \tfrac{1}{2}\log(1+\lambda)$,
      satisfying $h(0) = 0$, $h'(0) = 1/2$, and
      $h \to -\infty$ as $\lambda \to -1^+$.
      ...
      whence $\exp(r(\lambda)) = 1$ identically
      and $r \equiv 0$.
    \end{proof}
    ...
    \begin{proof}
      ...
      The endomorphism
      $X = f(R)\,\delta^{\mu}{}_{\nu}$
      has a single repeated eigenvalue.
      Two manifolds with equal scalar curvature but distinct
      Ricci eigenvalue distributions would produce
      identical~$\Phi$, violating the spectral resolution
      required by admissibility.
      The resulting completer
      $\Phi(X) = \exp\!\bigl(d \cdot h(f(R))\bigr)$ is a
      function of~$R$ alone, degenerate along all eigenvalue
      directions.
